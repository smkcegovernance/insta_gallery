...
export default function MessagesProvider(props: IDatabaseProviderProps) {
  ...
  const getMessage = React.useCallback(
    (id: number) => messages.find(message => message.id === id),
    [messages],
  );
  const addMessage = React.useCallback(
    (value: IMessage) => setMessages(_messages => [..._messages, value]),
    [],
  );
  const addMessages = React.useCallback(
    (value: IMessages) => setMessages(_messages => [..._messages, ...value]),
    [],
  );
